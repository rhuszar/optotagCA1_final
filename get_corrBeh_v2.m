...
save('corrBeh_v7.mat', 'pfcorr_ntag_all', 'pfcorr_ntag_left', 'pfcorr_ntag_right', 'pfcorr_ntag_dist', 'pfcorr_ntag_shankdist', ...
     ...
     'pfcorr_nolab_right', 'theta_nolab_rates', 'thetacorr_nolab', 'id_nolab', 'nolab_lRat', 'animal_nolab')

%% Spatial ratemap correlation of pyramidal cell pairs a function of convergence index

idx = converge_ntag > 0;

cnt = converge_ntag(idx);


var_ntag = pfcorr_ntag_all(idx,:);
var_tagtag = pfcorr_tagtag_all;

perc_step = 20;
perc_all = 0:perc_step:100;

[~, ~, ip] = histcounts(cnt, prctile( cnt, perc_all ));
...
ylabel('Spatial ratemap correlation')

shg

figure
histogram( cnt, 10, 'linestyle', 'none' )
hold on
xline( prctile( cnt, [20 40 60 80] ) )
