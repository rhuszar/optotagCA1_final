%% Exactly as the previous version (get_corrHome.m), except we consider ripples / theta cycles
%  in the homecage before and after the familiar track recordings
...
for ip = 1:length(basepaths_all)
    ...
    theta_ints = Theta.cycles;
    theta_ints = theta_ints( InIntervals( theta_ints(:,1), SleepState.ints.WAKEstate ) & InIntervals( theta_ints(:,2), SleepState.ints.WAKEstate ), : );  % Only keep theta cycles in the wake state
    theta_ints( theta_ints(:,1) > discard,: ) = [];
    theta_in_pulse = InIntervals( theta_ints(:,1), [ pulses.intsPeriods ; beh_ts ] ) | InIntervals( theta_ints(:,2), [ pulses.intsPeriods ; beh_ts ] );
    theta_ints(theta_in_pulse,:) = [];
    ...
    basenames_nolab = [basenames_nolab ; repmat({basename}, length(corrvec1_ripple), 1)];
...
save('corrHome_v7.mat', 'converge_ntag', 'theta_ntag_rates','ripple_ntag_rates', 'ntag_dist', 'ntag_shankdist','ripcorr_ntag', 'thetacorr_ntag', 'id_ntag', 'basenames_ntag', ...
     'converge_tagtag', 'ripcorr_tagtag', 'thetacorr_tagtag', 'theta_tagtag_rates', 'ripple_tagtag_rates', 'id_tag', 'basenames_tagtag', 'tagtag_dist', 'tagtag_shankdist', ...
     ...
     'nolab_dist', 'nolab_shankdist', 'ripcorr_nolab', 'thetacorr_nolab', 'id_nolab', 'animal_nolab', 'ripcorr_nolab_session', 'basenames_nolab')
